...
export function planBolIdentifierRegion(words,size){
  if(!Array.isArray(words)||words.length>100000)return null;
  ...
  for(const label of valid){
    ...
    const neighbors=valid.filter(w=>w!==label&&w.left>=label.left&&w.left+w.width<=label.left+size.width*.38&&Math.abs(w.top-label.top)<=label.height*.7);
    if(!neighbors.some(w=>/\d{3}/.test(w.text)))continue;
    ...
    const right=Math.max(label.left+label.width,...neighbors.map(w=>w.left+w.width));

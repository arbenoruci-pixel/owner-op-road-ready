...
export const rateConfirmationProfile={
  ...
  fields:{
    ...
    shipper:{label:'Shipper',kind:'party',required:false,pattern:/^\s*(?:SHIPPER|SHIP FROM)\s*:\s*(.+?)\s*$/id},
    consignee:{label:'Consignee',kind:'party',required:false,pattern:/^\s*(?:CONSIGNEE|SHIP TO)\s*:\s*(.+?)\s*$/id},
    pickupDate:{label:'Pickup date',kind:'date',required:false,pattern:/^\s*PICK\s*UP DATE\s*:\s*(.+?)\s*$/id},
    deliveryDate:{label:'Delivery date',kind:'date',required:false,pattern:/^\s*DELIVERY DATE\s*:\s*(.+?)\s*$/id},

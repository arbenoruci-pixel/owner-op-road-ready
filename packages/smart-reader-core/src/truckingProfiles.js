...
export function truckingProfiles(base){
  ...
  const profiles=[
    ...
    p('packing_list','Packing list','PACKING LIST|PACKING SLIP',[partySignal,sig('PACKING (?:LIST|SLIP) (?:NUMBER|NO)|QUANTITY|QTY|ITEM|SKU|PALLETS|CARTONS')],
      {...shipping,loadNumber:id('Load number','LOAD|SHIPMENT|PRO'),
        packingSlipNumber:{...id('Packing slip number','PACKING SLIP|PACKING LIST'),referenceRow:/^\s*PACKING (?:SLIP|LIST) (?:NUMBER|NO\.?)\s*[:;#]?\s*$/i},
        orderNumber:{...id('Order number','ORDER'),referenceRow:/^\s*ORDER (?:NUMBER|NO\.?)\s*[:;#]?\s*$/i},
        poNumber:{...id('PO number','(?:CUSTOMER )?(?:PO|P\\.O\\.)|PURCHASE ORDER'),referenceRow:/^\s*CUSTOMER P\.?\s*O\.?\s*[:;#]?\s*$/i},

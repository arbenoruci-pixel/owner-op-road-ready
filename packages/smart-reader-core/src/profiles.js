...
export const PROFILES = Object.freeze([
  ...
  },
  {
    ...
    signals:[/^\s*(?:(?:ship\s*from|shipper)\b|from\s*:)/im,/^\s*(?:(?:ship\s*to|consignee)\b|to\s*:)/im],
    ...
    identity:'bolNumber',
    fields:{
      bolNumber:{...field('BOL number',identifier('BOL|B/L|BILL OF LADING'),'identifier',true),checkIdentifierFragments:true,inlineLabel:/(?:^|\|)[ \t]*(?:BOL|B\/L|BILL OF LADING)[ \t]*(?:NUMBER\b|NO\b\.?|ID\b|#|:)[ \t:#;]*/i,rightLabel:/^\s*(?:BOL|B\/L|BILL OF LADING)\s*(?:NUMBER\b|NO\b\.?|#)\s*[:;#]*\s*$/i,noisyPattern:/^[\s"'|]*(?:BAL|BL)(?:\s*NO\.?\s*[:;]\s*|\s+(?=\d{6,20}\s*$))([A-Z0-9][A-Z0-9._/-]{2,39})\s*$/id},
      ...
      consignee:{...beside('Consignee',labeled('SHIP[ \\t]*TO|CONSIGNEE|CONSIGNED TO|TO(?=[ \\t]*:)'),'party',/^\s*(?:SHIP\s*TO|CONSIGNEE|CONSIGNED(?:\s+TO)?|TO(?=\s*:))\s*:?\s*$/i,true),blockLabel:/^\s*(?:SHIP\s*TO|CONSIGNEE)\s*[:.]*\s*$/i},
      ...
      poNumber:field('PO number',identifier('(?:(?:CUSTOMER|CUST)[ \\t]+)?(?:P\\.?[ \\t]*O\\.?|PURCHASE ORDER)'),'identifier'),

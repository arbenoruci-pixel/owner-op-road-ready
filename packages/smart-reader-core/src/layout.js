...
export function fieldMatches(lines,spec){
  ...
  for(const line of lines){
    ...
    if(inline){
      ...
      if(spec.kind==='party'&&STREET_ADDRESS.test(value))continue;
    ...
    if(rightLabel&&line.box){
      ...
      const first=right[0];
      if(first){const range=valueRange(first);if(range&&range[1]>range[0]&&!(spec.kind==='party'&&STREET_ADDRESS.test(first.text)))matches.push({line:first,start:range[0],end:range[1],labelLine:line,issue:'layout_needs_review'});continue;}
    }

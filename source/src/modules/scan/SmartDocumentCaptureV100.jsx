...
const SCAN_TYPES = [
  { id:'auto', label:'Auto detect', detail:'BOL, Rate Con, fuel, settlement or receipt', icon:'spark' },
  ...
  { id:'rate_confirmation', label:'Rate Con', detail:'Load, broker, route, dates and rate', icon:'money' },
  { id:'fuel_receipt', label:'Fuel / Mudflap', detail:'Mudflap PDF, fuel receipt, gallons and IFTA state', icon:'fuel' },
  { id:'carrier_settlement', label:'Settlement', detail:'Gross, deductions and net pay', icon:'percent' },
...
function Icon({ name, size = 23 }) {
  ...
  if (name === 'file') return <svg {...common}><path d="M6 3h9l3 3v15H6zM15 3v4h4" /><path d="M9 12h6M9 16h6" /></svg>;
  if (name === 'chevron') return <svg {...common}><path d="m9 18 6-6-6-6" /></svg>;
  return <svg {...common}><circle cx="12" cy="12" r="9" /></svg>;
}

export default function SmartDocumentCaptureV100({ onReady, onClose }) {
  ...
  function openScanner(file = null) {
    setInitialFile(file);
    setScannerOpen(true);
  }
  ...
  function importFile(file) {
    ...
    onReady?.(file, preferredType, {
      source:'pro-file-import-v100',
      imported:true,
      pageCount:1,
      preferredType,
  ...
  if (scannerOpen) {
    return (
      <TurboDocumentScanner
        ...
        onComplete={(file, metadata = {}) => onReady?.(file, preferredType, { ...metadata, preferredType, originalFileName:initialFile?.name || file?.name || '' })}
      />
    );
  }

  return (
    <section className="scan-preflight-screen pro-import-v100">
      <header className="scan-preflight-head">
        <button type="button" onClick={onClose} aria-label="Close scanner"><Icon name="close" /></button>
        <div><b>Pro Document Inbox</b><em>Camera · Photos · PDF · Files</em></div>
        <span />
      </header>
      ...
      <main className="scan-preflight-body">
        <div className="scan-preflight-hero">
          ...
          <p>Document-aware reader</p>
          <h1>Import the real trucking document</h1>
        ...
        <div className="scan-preflight-grid" role="listbox" aria-label="Document type">
          {SCAN_TYPES.map(item => (
            <button key={item.id} type="button" role="option" aria-selected={preferredType === item.id} className={preferredType === item.id ? 'selected' : ''} onClick={() => setPreferredType(item.id)}>
              <span><Icon name={item.icon} /></span><b>{item.label}</b><em>{item.detail}</em><i>{preferredType === item.id ? '✓' : ''}</i>
            </button>
          ))}
        </div>
      ...
      <footer className="scan-preflight-actions pro-four-v100">
        <button type="button" className="primary" onClick={() => openScanner(null)}><Icon name="camera" /> Best quality scan <Icon name="chevron" size={18} /></button>
        <button type="button" className="native" onClick={() => phoneCameraRef.current?.click()}><Icon name="camera" /> Phone camera</button>
        <button type="button" className="library" onClick={() => photoLibraryRef.current?.click()}><Icon name="gallery" /> Choose Photos</button>
        <button type="button" className="file-import-v100" onClick={() => fileImportRef.current?.click()}><Icon name="file" /> Import PDF / File</button>
        <input ref={phoneCameraRef} className="smart-scan-file-input" type="file" accept="image/*" capture="environment" onChange={event => { const file = event.target.files?.[0] || null; event.target.value = ''; if (file) openScanner(file); }} />
        <input ref={photoLibraryRef} className="smart-scan-file-input" type="file" accept="image/*" onChange={event => { const file = event.target.files?.[0] || null; event.target.value = ''; if (file) openScanner(file); }} />
        <input ref={fileImportRef} className="smart-scan-file-input" type="file" accept="application/pdf,image/*,text/plain,text/csv,.pdf,.txt,.csv" onChange={event => { const file = event.target.files?.[0] || null; event.target.value = ''; importFile(file); }} />
      </footer>
    </section>
  );
}

...
function normalizeState(s) {
  ...
  const currentStatusForSession = currentFromRaw?.status || safePersistedStatus || currentFromCarry?.status || 'OFF';
  const persistedManualSession = s.manualDrivingSession || null;
  const normalizedManualDrivingSession = persistedManualSession && currentStatusForSession !== 'D' && persistedManualSession.active === true
    ? {
      ...persistedManualSession,
      active:false,
      endedByStatus:currentStatusForSession,
      endedDay:today,
      endedAt:persistedManualSession.endedAt || new Date().toISOString(),
  ...
  const normalized = {
    ...
    manualDrivingSession:normalizedManualDrivingSession,
...
function reconcileCurrentManualDrivingContinuity(saved = {}, now = new Date(), options = {}) {
  const zone = getHomeTerminalTimeZone(saved);
  const currentDay = localDayKey(now, zone);
  const previousDay = addDays(currentDay, -1);
  const nowMinute = homeTerminalMinute(now, zone);
  return applyManualDrivingMidnightContinuity(saved, {
    currentDay,
    previousDay,
    nowMinute,
    allowClosedGapRepair:options.allowClosedGapRepair !== false,
    forceActiveDriving:options.forceActiveDriving === true,
    reason:options.reason || 'current_day_manual_driving_continuity',
  });
}
...
async function loadInitial() {
  try {
    ...
    if (saved) {
      ...
      const corrected = applyUserConfirmedHubbardSleeperStopRepair(continuous);
...
export default function App() {
  ...
  function rolloverActiveDrivingIfNeeded(s, now = new Date()) {
    // Manual Driving is a driver-entered duty status. If it crosses midnight,
    // split the same active status at 00:00 before any new status is saved.
    // This fills only an uncovered start-of-day gap and never replaces rows.
    const base = reconcileCurrentManualDrivingContinuity(s, now, {
      allowClosedGapRepair:false,
      forceActiveDriving:s.currentStatus === 'D',
      reason:'live_manual_driving_midnight_rollover',
    });

    const trip = base.gpsTrip || null;
    if (!trip || trip.status !== 'active' || !trip.eventId) return base;

    const tz = getHomeTerminalTimeZone(base);
    const today = localDayKey(now, tz);
    const nowMinute = minuteFromDate(now, base);
    const gpsId = trip.eventId;
    const sourceDay = findEventDayById(base.eventsByDay || {}, gpsId);

    // A stale GPS session may never become a broad midnight DRIVING row. The
    // manual continuity pass above is independent of GPS and only follows an
    // explicitly active driver-entered D status.
    if (!sourceDay || sourceDay > today || sourceDay !== base.activeDay) {
      return {
        ...base,
        gpsTrip:{ ...trip, status:'stale', staleReason:'gps_event_day_mismatch', staleAt:Date.now() },
      };
    }
    if (sourceDay === today) return base;

    const previousBase = continuousBaseForDay(base, sourceDay);
    ...
    const todayExisting = rawStoredEventsForDay(base.eventsByDay || {}, today);
    const todayDrive = {
      ...sourceEvent,
      id:rolloverId,
      status:'D',
      ...
      city:base.currentLocation?.city || sourceEvent.city || 'GPS',
      state:base.currentLocation?.state || sourceEvent.state || 'UNK',
      note:'Driving',
    ...
    const todayUpdated = commitTimelineForDay(safeToday, today, base);
    ...
    return {
      ...
      currentStatus:'D',
      currentReason:base.currentReason || 'Driving',
      currentLocation:{
        ...(base.currentLocation || {}),
        city:base.currentLocation?.city || todayDrive.city,
        state:base.currentLocation?.state || todayDrive.state,
      },
      certifyStatus:{ ...(base.certifyStatus || {}), [today]:base.certifyStatus?.[today] || 'Active day / Not certified yet' },
      eventsByDay:{ ...(base.eventsByDay || {}), [sourceDay]:previousUpdated, [today]:todayUpdated },
  ...
  function normalizeRouteLeg(leg = {}) {
    const kind = leg.kind || ((leg.emptyMove || leg.mode === 'hook_empty' || /empty|reposition|return/i.test(String(leg.source || ''))) ? 'empty/reposition' : 'loaded');
    const docs = kind === 'empty/reposition' && !/^(EMPTY|MT|EMPTY\/MT)$/i.test(String(leg.shippingDocs || leg.loadNo || '').trim())
      ? ''
      : (leg.shippingDocs || leg.loadNo || '');
    return {
      ...
      seal: leg.seal || leg.hookedSeal || '',
      droppedContainer: leg.droppedContainer || '',
      droppedChassis: leg.droppedChassis || '',
      deliveredLoadNo: leg.deliveredLoadNo || '',
      pickedUpLoadNo: leg.pickedUpLoadNo || '',
      transitionLoadNos: Array.isArray(leg.transitionLoadNos) ? leg.transitionLoadNos : [],
      kind,
  ...
  function isHookEmptyReason(reason = '', dropHook = {}) {
    return /\b(hook\s+empty|empty\s+return|reposition|return\s+empty)\b/i.test(String(reason || '')) || String(dropHook?.mode || '') === 'hook_empty';
  }

  function isIntermodalDropReason(reason = '', dropHook = {}) {
    return isDropHookReason(reason, dropHook) || isDropOffReason(reason, dropHook) || isHookEmptyReason(reason, dropHook);
  }
  ...
  function buildDropHookNote(payload = {}, currentEquipment = {}) {
    ...
    const hookEmpty = isHookEmptyReason(payload.reason, dropHook);
    ...
    if (!dropOnly && hooked) parts.push(hookEmpty ? `Hooked empty ${hooked}` : `Hooked ${hooked}`);
    if (!dropped && !hooked) parts.push(dropOnly ? 'Equipment dropped' : (hookEmpty ? 'Empty equipment hooked' : 'Equipment changed'));
    if (!dropOnly && !hookEmpty && dropHook.hookedLoadNo) parts.push(`BOL ${dropHook.hookedLoadNo}`);
    if (!dropOnly && hookEmpty && dropHook.hookedLoadNo) parts.push(`Ref ${dropHook.hookedLoadNo}`);
    if (!dropOnly && dropHook.hookedDestination) parts.push(`to ${dropHook.hookedDestination}`);
    const prefix = dropOnly ? 'Drop Off' : (hookEmpty ? 'Hook Empty / Reposition' : 'Drop & Hook');
  ...
  function buildEquipmentFromDropHook(equipment = {}, payload = {}) {
    ...
    return {
      ...
      note: sanitizeLogText([
        ...
        !dropOnly && !hookEmpty && dropHook.hookedLoadNo ? `BOL ${dropHook.hookedLoadNo}` : '',
        !dropOnly && hookEmpty && dropHook.hookedLoadNo ? `Ref ${dropHook.hookedLoadNo}` : '',
        !dropOnly && hookEmpty ? 'Empty/reposition move' : '',
  ...
  function buildDropHookLoadPatch(payload = {}, eventId = '') {
    ...
    if (dropOnly) {
      return {
        ...
        equipmentChassis:'',
        equipmentSeal:'',
        currentMoveKind:'drop_off',
      };
    }
    const nextDestination = parseCityStateInput(dropHook.hookedDestination || payload.destination || '', payload.destinationState || '');
    const nextLoadNo = String(dropHook.hookedLoadNo || '').trim();
    return {
      sourceEventId:eventId,
      sourceEventReason:payload.reason || (hookEmpty ? 'Hook Empty / Reposition' : 'Drop & Hook'),
      ...
      currentMoveKind:hookEmpty ? 'empty/reposition' : 'loaded',
    };
  }
  ...
  function updateRouteLegsForDropHook(routeLegsByDay = {}, day, payload = {}, eventId = '', startMin = 0, stateBefore = {}) {
    ...
    const hookEmpty = isHookEmptyReason(payload.reason, dropHook);
    const dropOnly = isDropOffReason(payload.reason, dropHook) && !dropHook.hookedContainer && !dropHook.hookedChassis;
    const nextDocs = String(dropHook.hookedLoadNo || '').trim();
    const emptyChoiceDocs = /^(EMPTY|MT|EMPTY\/MT)$/i.test(nextDocs) ? nextDocs : '';
    const nextDestination = parseCityStateInput(dropHook.hookedDestination || payload.destination || '', payload.destinationState || '');
    ...
    if (openForOldLoad) {
      next = upsertRouteLeg(next, openForOldLoad.day || openForOldLoad.pickupDay || day, {
        ...
        deliveredLoadNo:oldDocs,
        transitionLoadNos:[oldDocs, hookEmpty ? emptyChoiceDocs : nextDocs].filter(Boolean),
        status:'delivered',
        updatedAt:Date.now(),
      });
    }

    if (!dropOnly && (dropHook.hookedContainer || dropHook.hookedChassis || nextDocs || nextDestination.city || nextDestination.state)) {
      next = upsertRouteLeg(next, day, {
        ...
        shippingDocs:hookEmpty ? emptyChoiceDocs : nextDocs,
        loadNo:hookEmpty ? emptyChoiceDocs : nextDocs,
        ...
        droppedChassis:dropHook.droppedChassis || stateBefore.equipment?.chassis || '',
        deliveredLoadNo:oldDocs,
        pickedUpLoadNo:hookEmpty ? emptyChoiceDocs : nextDocs,
        transitionLoadNos:[oldDocs, hookEmpty ? emptyChoiceDocs : nextDocs].filter(Boolean),
        kind:hookEmpty ? 'empty/reposition' : 'loaded',
        status:'open',
        source:hookEmpty ? 'hook_empty_reposition_event' : 'drop_hook_event', // source:'drop_hook_event' legacy verifier marker
        updatedAt:Date.now(),
      });
    }
    return next;
  ...
  function addDriverWorkflowEvents(kind) {
    ...
    setState(s => {
      ...
      routeLegsByDay = syncRouteLegTimes(routeLegsByDay, eventsByDay);
      const workflowDrivingEvent = [...toAdd].reverse().find(event => event.status === 'D') || null;
      const manualDrivingSession = currentStatus === 'D'
        ? {
          active:true,
          status:'D',
          eventId:workflowDrivingEvent?.id || s.manualDrivingSession?.eventId || '',
          startDay:day,
          startedAt:new Date().toISOString(),
          location:{ city:currentLocation?.city || '', state:currentLocation?.state || '' },
          source:'driver_workflow',
        }
        : (s.manualDrivingSession?.active
          ? {
            ...s.manualDrivingSession,
            active:false,
            endedDay:day,
            endedMin:now,
            endedByStatus:currentStatus,
            endedAt:new Date().toISOString(),
          }
          : (s.manualDrivingSession || null));
  ...
  function closeLastAndAddStatus({ status, reason, city, state: st, description='', droppedTrailer='', hookedTrailer='', dropHook=null, lat=null, lng=null, gpsAccuracy=null, locationSource='manual', shippingDocs='', loadNo='', destination='', destinationState='', backdateMinutes=0 }) {
    ...
    setState(s => {
      ...
      const nowLiveMin = minuteFromDate(now, base);
      ...
      let note = reason;
      let trailer = base.currentTrailer;
      ...
      if (isIntermodalDrop) {
        note = buildDropHookNote({ reason, city, state:st, dropHook }, base.equipment || {});
      ...
      const isHookEmpty = isHookEmptyReason(reason, dropHook);
      const effectiveShippingDocs = (isDropHook || isHookEmpty)
        ? String(dropHook?.hookedLoadNo || '').trim()
        : String(shippingDocs || loadNo || '').trim();
      const loadDescription = isIntermodalDrop
        ? buildDropHookNote({ reason, city, state:st, dropHook }, base.equipment || {})
      ...
      const ev = {
        ...
        startMin:changeAt,
        endMin:Math.max(changeAt + 1, Math.min(1440, nowLiveMin || changeAt + 1)),
        city:effectiveCity,
      ...
      const eventsByDay = { ...(base.eventsByDay || {}), [day]:continuous };
      const routeBase = isIntermodalDrop
        ? updateRouteLegsForDropHook(base.routeLegsByDay || {}, day, loadPayload, eventId, changeAt, base)
        : updateRouteLegsForStatus(base.routeLegsByDay || {}, day, loadPayload, eventId, changeAt);
      ...
      const gpsTrip = base.gpsTrip?.status === 'active'
        ? { ...base.gpsTrip, status:'stale', staleReason:'manual_status_change', staleAt:Date.now() }
        : base.gpsTrip;
      const priorManualSession = base.manualDrivingSession || null;
      const manualDrivingSession = status === 'D'
        ? {
          active:true,
          status:'D',
          eventId,
          ...
          location:{ city:effectiveCity, state:effectiveState },
      ...
      let next = {
        ...
        selectedEventId:null,
        sheet:null,
        view:status === 'D' ? 'driveMode' : (base.view === 'driveMode' ? 'day' : base.view),
        eventsByDay,
        routeLegsByDay,
  ...
  function stopDrivingToOnDuty(fix = null) {
    setState(s => {
      ...
      return {
        ...
        manualDrivingSession:base.manualDrivingSession ? {
          ...base.manualDrivingSession,
          active:false,
          endedAt:now.toISOString(),
          endedDay:day,
          endedMin:changeAt,

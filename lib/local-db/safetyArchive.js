...
export async function buildDeviceSafetyArchive({ state = {}, businessStore = {}, appVersion = '', onProgress = () => {} } = {}) {
  const db = getOwnerOpDb();
  ...
  const dexie = {};
  for (const table of db.tables) {
    onProgress(`Preparing backup: ${Object.keys(dexie).length + 1} of ${db.tables.length} record groups…`);
  ...
  onProgress('Checking the complete backup…');

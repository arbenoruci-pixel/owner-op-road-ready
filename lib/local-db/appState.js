...
export const PRE_CLOUD_RAW_STATE_KEY = 'owner-op-road-ready-pre-cloud-raw-v1';
export const UPDATE_META_KEY = 'owner-op-road-ready-update-meta-v1';

async function preservePreCloudRawSnapshot(db, state) {
  if (!db || !state) return;
  const now = new Date().toISOString();
  try {
    const existing = await db.app_snapshots.get(PRE_CLOUD_RAW_STATE_KEY);
    if (!existing?.state) {
      const payload = {
        ...state,
        _backupMeta: {
          kind: 'pre_cloud_raw_snapshot',
          createdAt: now,
          immutableIntent: true,
        },
      };
      await db.app_snapshots.put({ key: PRE_CLOUD_RAW_STATE_KEY, state: payload, updated_at: now });
      await db.sync_meta.put({ key: 'pre_cloud_raw_snapshot_at', value: now, updated_at: now });
      try {
        if (typeof window !== 'undefined' && window.localStorage && !window.localStorage.getItem(PRE_CLOUD_RAW_STATE_KEY)) {
          window.localStorage.setItem(PRE_CLOUD_RAW_STATE_KEY, JSON.stringify(payload));
        }
      } catch {}
    }
  } catch {}
...
export async function loadAppSnapshot(key = APP_STATE_KEY) {
  ...
  if (key === APP_STATE_KEY && row?.state) await preservePreCloudRawSnapshot(db, row.state);
...
export async function loadDutyEventRecoveryHistory(day) {
  ...
  try {
    const [allRevisions, allMaps] = await Promise.all([

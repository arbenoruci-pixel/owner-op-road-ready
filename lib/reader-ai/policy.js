...
export function aiClassificationReason(identity, text = '') {
  ...
  // An unloading/sign-out stamp can require visual delivery review even when
  // OCR recognizes only its printed label, not the handwritten completion.
  if (identity.kind === 'bol' && /^\s*UNLOADED\s+(?:&|AND)\s+SIGNED\s+OUT\s*:/im.test(text)) return 'delivery_acknowledgement';
  return null;
}

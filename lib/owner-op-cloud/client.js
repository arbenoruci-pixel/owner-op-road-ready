...
const SAFETY_EXPORT_META_KEY = 'owner-op-road-ready-last-device-safety-export-v1';
...
function hasVerifiedDeviceSafetyBackup() {
  try {
    const row = JSON.parse(localStorage.getItem(SAFETY_EXPORT_META_KEY) || 'null');
    return !!(row?.createdAt && row?.filename && /^[a-f0-9]{64}$/.test(String(row?.sha256 || '')) && Number(row?.bytes || 0) > 0);
  } catch {
    return false;
  }
}
...
export async function backupLocalData({ maxUploads = 20, onProgress = () => {}, bootstrap = false, onlyDays = null } = {}) {
  ...
  if (navigator.onLine === false) throw new Error('Offline. Your local records remain on this device.');
  if (!hasVerifiedDeviceSafetyBackup()) throw new Error('Cloud backup is locked. Create and save a VERIFIED Device Safety Backup from Data Safety first.');

...
function replaceOnce(content, before, after, label) {
  ...
  if (!content.includes(before)) throw new Error(`v105 missing ${label}`);
  return content.replace(before, after);
}
function replaceRegex(content, pattern, replacement, marker, label) {
  if (marker && content.includes(marker)) return content;
  if (!pattern.test(content)) throw new Error(`v105 missing ${label}`);
...
// The focused three-field Scan flow owns production. The v104 document engine
// remains the reader; v105 adds safe identity, matching and filing around it.
...
// Keep Vault editing on the same trucking document catalog as the reader.
...
// Canonical-state repair and safe document commits are wired into the app shell.
...
app = replaceRegex(
  ...
  'onVaultDocumentCommit',
  'Vault document listener',
);
write(appPath, app);

// Documents becomes a first-class screen. Home enriches its load checklist from
// the Vault instead of treating a load reference as if it were a saved BOL.
...
// Active-load Home exposes the Vault and checks the central BOL checklist.
...
// New and existing profiles receive the Documents module without losing their
// selected modules.
...
profile = profile
  .replace("leased_on:['loads','settlements'", "leased_on:['loads','documents','settlements'")
...
// Install the new UI without replacing the established design system.
const commandCssPath = 'source/src/command-center.css';
write(commandCssPath, appendOnce(read(commandCssPath), '/* v105 Road Ready OS Foundation */', read('source/src/road-ready-os-v105.css')));
...
for (const [relative, marker] of [
  ...
  [commandCssPath,'v105 Road Ready OS Foundation'],
]) {

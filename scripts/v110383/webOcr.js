...
let completedCache = new WeakMap();
function bounded(promise, ms, code, signal) {
  let timer;
  return withAbort(Promise.race([promise, new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error(code)), ms);
  })]), signal).finally(() => clearTimeout(timer));
}

function aborted() { return new DOMException('Reading canceled', 'AbortError'); }
function withAbort(promise, signal, stop) {
...
async function worker(signal) {
  const Tesseract = await withAbort(loadScript(), signal);
  if (!workerPromise) {
    ...
    const current = bounded(pendingWorker, 45000, 'ocr_model_timeout', signal).then(async instance => {
      try {
        ...
        }), 8000, 'ocr_parameters_timeout', signal);
      } catch {}
      return instance;
    }).catch(error => {
    ...
    // Keep the raw creation promise so cancellation can stop a late-created
    // worker immediately, without touching a replacement worker's promise.
    current.creation = pendingWorker;
    workerPromise = current;
  }
  const current = workerPromise;
  return withAbort(current, signal, () => {
    if (workerPromise === current) workerPromise = null;
    (current.creation || current).then(instance => instance.terminate()).catch(() => {});
  });
}
...
export async function recognizeDocumentText(file, options = {}) {
  ...
  const cached = completedCache.get(file)?.get(cacheKey) || !signal && resultCache.get(file)?.get(cacheKey);
  if (cached) { onProgress(1, 'Using text already read from this image'); return cached; }
  const cacheGeneration = completedCache;
  const task = runSerial(async () => {
    if (signal?.aborted) throw aborted();
    // Another queued request may have completed while this one waited. Only
    // successful results are shared; cancellation stays local to each caller.
    const ready = completedCache.get(file)?.get(cacheKey);
    if (ready) { onProgress(1, 'Using text already read from this image'); return ready; }
    activeProgress = onProgress;
    let engine;
    try {
      engine = await worker(signal);
      ...
      const result = await bounded(engine.recognize(file, recognizeOptions, outputOptions), 35000, 'ocr_read_timeout', signal);
  ...
  const finished = task.then(value => {
    if(value && !signal?.aborted && completedCache === cacheGeneration){
      let entries=completedCache.get(file);
      if(!entries){entries=new Map();completedCache.set(file,entries);}
      if(entries.size>=8&&!entries.has(cacheKey))entries.delete(entries.keys().next().value);
      entries.set(cacheKey,value);
    }
    return value;
  });
  if (signal) return withAbort(finished, signal);
  let entries=resultCache.get(file);
  if(!entries){entries=new Map();resultCache.set(file,entries);}
  entries.set(cacheKey,finished);
  finished.catch(()=>{if(entries.get(cacheKey)===finished)entries.delete(cacheKey);});
  return finished;
}

export async function terminateWebOcr() {
  resultCache = new WeakMap();
  completedCache = new WeakMap();

...
test('historical Form cannot inherit a global current load',()=>{
...
test('recorded day evidence wins while stale global load stays isolated',()=>{
...
test('GPS state-only response never promotes a county subdivision to city',()=>{
  ...
  return resolveGpsPosition(position,{fetchImpl:async()=>({
    ok:true,
...
test('server returns county subdivision as diagnostic only',()=>{});
...
test('generic load metadata save is fenced from RODS writes',()=>{
...
test('paper Form route edits declare exact log-day scope',()=>{
...
test('GPS provenance survives the Status save',()=>{

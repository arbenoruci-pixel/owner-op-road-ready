...
const BUILD='v110305-live-bol-context-force-update';
...
// Context fallback is deliberately narrow: only a weak/uncertain Rate Con or
// Other result may inherit BOL identity from the latest active pickup event.
// Strong Rate Con evidence and every other confidently detected document type
...

{
  ...
  source=once(source,
    ...
    "    const extractedLoad = primaryLoadReference(result);\n    const liveContextLoadV11035 = normalizeCanonicalLoadNoV105(result.liveBolContextV11035?.loadNo || '');",
    'live contextual load');
  source=once(source,
...
`    const loadNo = preferredLoadNo
      || nextMatch.loadNo
      || liveContextLoadV11035
      || rateConNewLoad
      || '';`,
    'current BOL load fallback');
  source=once(source,
    "            {match?.reason ? <em>{selectedLoadNo === match.loadNo ? match.reason : 'Driver-selected load folder'}</em> : null}",
    "            {analysis?.liveBolContextV11035 ? <em>Current live pickup BOL · verify before saving</em> : match?.reason ? <em>{selectedLoadNo === match.loadNo ? match.reason : 'Driver-selected load folder'}</em> : null}",
    'context explanation');
  write(path,source);
}

// Forced versions cannot be hidden after they are detected. This keeps an old
// PWA shell from sitting on a known-bad scanner build after the driver returns
// to the app.
{
...
`        const available = Boolean(remote.version)
          && isNewerVersion(remote.version, CURRENT_APP_VERSION)
          && (remote.force || prev.dismissedVersion !== remote.version);`,
    'forced update availability');
  source=once(source,
...
`  function dismissAppUpdate() {
    setUpdateState(prev => {
      if (prev.remote?.force) return prev;
      return {
        ...prev,
        available: false,
        dismissedVersion: prev.remote?.version || prev.latestVersion || '',
...
{
  const path='source/src/modules/update/UpdateBanner.jsx';
  let source=read(path);
  source=once(source,
    "  const busy = updateState?.saveState === 'saving-update';",
    "  const busy = updateState?.saveState === 'saving-update';\n  const forced = updateState?.remote?.force === true;",
    'forced banner state');
  source=once(source,
    "        <b>{busy ? 'Saving before update' : 'Update ready'}</b>",
    "        <b>{busy ? 'Saving before update' : forced ? 'Update required' : 'Update ready'}</b>",
    'forced banner title');
  source=once(source,
    "        {!busy ? <button type=\"button\" className=\"update-safe-secondary\" onClick={onDismiss}>Later</button> : null}",
    "        {!busy && !forced ? <button type=\"button\" className=\"update-safe-secondary\" onClick={onDismiss}>Later</button> : null}",
    'forced banner no later');
  write(path,source);
}
...
for(const path of ['release-version.json','public/app-version.json']){
  ...
  Object.assign(meta,{version:VERSION,build:BUILD,force:true,releasedAt,updatedAt:releasedAt,label:'v110.3.5 Live BOL Context Recovery',notes:[
    'A weak or uncertain Rate Confirmation result is re-evaluated against the current live pickup BOL before the confirmation screen is shown.',
    'The current live pickup BOL supplies the active load folder and pickup-day date only when scanner evidence is weak; strong Rate Confirmations and other document types remain unchanged.',
    'Forced PWA updates cannot be dismissed after this build is installed.'
  ]});
...
if(fs.existsSync(lockPath)){
  const locks=JSON.parse(read(lockPath));locks.release=VERSION;write(lockPath,JSON.stringify(locks,null,2)+'\n');
}
console.log('PASS — 110.3.5 live BOL context fallback and forced-update guard applied');

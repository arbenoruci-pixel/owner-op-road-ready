...
async function openLog(page){
 // Startup deliberately resumes Drive Mode for an active D session. Navigate
 // through its real Log control instead of changing app startup behavior.
 await page.locator('.drive-mode-log-btn, .logbook-ui-v110 .log-graph-v110').first().waitFor();
 if(await page.locator('.drive-mode-log-btn').isVisible())await page.locator('.drive-mode-log-btn').click();
...
async function seed(page,state,existing=false){await page.goto(origin+'/_not-found');await page.evaluate(async({schemas,state,session,existing})=>{
 ...
 },{schemas,state,session,existing});await page.goto(origin);await openLog(page);}
...
for(const [name,type] of [['chromium',chromium],['webkit',webkit]])for(const zone of ['America/Los_Angeles','Europe/Belgrade']){
 ...
 try{
  ...
  await page.screenshot({path:`browser-test-results/${prefix}-log.png`,fullPage:false});
  ...
  console.log('PASS — '+prefix+' closed edit/cancel/save/reload/24:00; starting live');
  ...
  const liveContrast=await inspect(page,prefix+' live');await page.screenshot({path:`browser-test-results/${prefix}-live-editor.png`,fullPage:false});
  ...
  await page.reload();await openLog(page);await page.locator('[data-log-event-id=live] .blue-edit').click();assert.equal((await page.locator('.selected-duration-live b').innerText()).trim(),'2h 5m');assert.match(await page.locator('.note-toggle-v90').innerText(),/Live note proof/);

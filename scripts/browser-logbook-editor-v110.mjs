...
async function seed(page,state){await page.goto(origin+'/_not-found');await page.evaluate(async({schemas,state,session})=>{
 ...
 },{schemas,state,session});await page.goto(origin);await page.locator('.logbook-ui-v110 .log-graph-v110').waitFor();}
...
for(const [name,type] of [['chromium',chromium],['webkit',webkit]])for(const zone of ['America/Los_Angeles','Europe/Belgrade']){
 ...
 try{
  ...
  const live=fixture(true);await seed(page,live);await page.locator('[data-log-event-id=live] .blue-edit').waitFor();const liveBefore=await stored(page);

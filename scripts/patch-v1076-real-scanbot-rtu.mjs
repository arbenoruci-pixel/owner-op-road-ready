...
function timeoutV1076(promise, ms, code) {
...
function loadUiV1076() {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.onload = () => window.ScanbotSDK?.UI?.createDocumentScanner
      ? resolve(window.ScanbotSDK)
      : reject(new Error('professional_scanner_ui_missing'));
...
export async function runScanbotRtuV1076(options = {}) {
  ...
  for (let index = 0; index < result.document.pages.length; index += 1) {
    ...
    const originalBytes = await sdk.imageToJpeg(originalImage);
    const finalBytes = await sdk.imageToJpeg(finalImage);
...
const effect = `  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!result) {
          setProfessionalStateV1076('cancelled');
          onCancel?.();
          return;
        }
        setProfessionalMessageV1076('Preparing Road Ready document…');
        const originalOutput = result.originals.length === 1
          ? result.originals[0]
          : await composePageFiles(result.originals, \`road-ready-scanbot-original-packet-\${Date.now()}.jpg\`);
        const ocrFile = result.cleaned.length === 1
          ? result.cleaned[0]
          : await composePageFiles(result.cleaned, \`road-ready-scanbot-cleaned-packet-\${Date.now()}.jpg\`);
        if (disposedV1076) return;
        setProfessionalStateV1076('complete');
        onComplete?.(originalOutput, {
          source:'scanbot-rtu-v1076',
          scannerVersion:'107.6.0',
          professionalScanner:'scanbot-web-sdk',
          professionalScannerVersion:SCANBOT_RTU_VERSION_V1076,
  ...
  if (professionalStateV1076 !== 'complete') {
    return <section data-professional-scanner="scanbot-rtu-v1076" style={{ minHeight:'100dvh', background:'#070707', color:'#fff', display:'grid', placeItems:'center', padding:24 }}>
      <div style={{ width:'min(520px,100%)', textAlign:'center' }}>
        ...
        {professionalStateV1076 === 'error' ? <>
          <p style={{ color:'#fca5a5', fontWeight:800 }}>The old Road Ready edge scanner is disabled. No hidden fallback was used.</p>
          <button type="button" onClick={onCancel} style={{ marginTop:18, border:0, borderRadius:14, padding:'14px 22px', fontWeight:900 }}>Close</button>
        </> : <div aria-label="Loading professional scanner" style={{ width:52, height:52, border:'5px solid #334155', borderTopColor:'#3b82f6', borderRadius:'50%', margin:'0 auto', animation:'spin 1s linear infinite' }}/>} 
      </div>
    </section>;
  }
`;
...
for (const marker of ['real-scanbot-rtu-v1076','runScanbotRtuV1076({','data-professional-scanner="scanbot-rtu-v1076"','No hidden fallback was used']) {
  if (!source.includes(marker)) throw new Error(`v107.6 missing ${marker}`);
}
write(target, source);

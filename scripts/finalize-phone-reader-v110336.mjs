...
import {installPhoneReaderV110336} from './v110336/install.mjs';
installPhoneReaderV110336();
const VERSION='110.3.36',BUILD='v110336-phone-capture-reader-guards';
for(const path of ['release-version.json','public/app-version.json']){
  const value=JSON.parse(fs.readFileSync(path,'utf8'));
  Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.36 Camera finish and reader checks',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,notes:['Keep a held sheet locked when the phone moves.','Allow Done during processing and finish the current photo before closing.','Reject signature instructions as party names and match complete identifier labels.']});
...
const before="assert.equal(meta.version,'110.3.35');assert.equal(meta.build,'v110335-owned-reader-preview');";
...
console.log('PASS — v110.3.36 camera finish and document field guards installed');

...
import './v110378/finish-interaction.mjs';
import fs from 'node:fs';
import assert from 'node:assert/strict';
import {spawnSync} from 'node:child_process';
const VERSION='110.3.78',BUILD='v110378-unified-duty-recorded-trips';
const read=path=>fs.readFileSync(path,'utf8');
const result=spawnSync(process.execPath,['scripts/v110378/test-duty-forms.mjs'],{stdio:'inherit'});
...
for(const path of ['scripts/test-duty-graph-continuity.mjs','scripts/test-editor-grips-v110355.mjs','scripts/verify-log-integrity-v1051.mjs','scripts/test-document-continuity-integration-v110375.mjs'])fs.writeFileSync(path,read(path).replaceAll("'110.3.77'","'"+VERSION+"'").replaceAll("'v110377-fast-secure-session'","'"+BUILD+"'"));
...
console.log('PASS — 110.3.78 release preserves prior startup, Reader and day-isolation changes');

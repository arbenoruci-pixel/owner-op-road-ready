...
import './v110371/install-browser-regressions.mjs';
...
const VERSION='110.3.75',BUILD='v110375-document-continuity-integrated';
const read=path=>fs.readFileSync(path,'utf8');
for(const path of ['release-version.json','public/app-version.json']) {
 const value=JSON.parse(read(path));Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.75 Saved reading continuity',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,
...
for(const path of ['scripts/test-duty-graph-continuity.mjs','scripts/test-editor-grips-v110355.mjs','scripts/verify-log-integrity-v1051.mjs'])fs.writeFileSync(path,read(path).replaceAll("'110.3.74'","'"+VERSION+"'").replaceAll("'v110374-recorded-route-day'","'"+BUILD+"'"));
...
console.log('PASS — 110.3.75 document continuity integrated with Reader evidence and route-day isolation; protected module hashes unchanged');

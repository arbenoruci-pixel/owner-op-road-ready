...
async function seed(page,state){
 ...
 await page.goto(origin);await page.getByText('Road Ready',{exact:true}).first().waitFor({timeout:30000});
}
...
for(const[name,type]of[['chromium',chromium],['webkit',webkit]]){
 ...
 try{
  for(const [kind,state,expectedActive] of [['fake-log-route',baseState(),false],['ratecon-guide',rateConState(),true]]){
   ...
   try{await seed(page,state);const body=await page.locator('body').innerText();assert.doesNotMatch(body,/live_1788780557647/i,'operational event ID leaked into Home load identity');if(expectedActive){await page.locator('.adaptive-mission-v1038').waitFor();assert.match(await page.locator('.adaptive-mission-v1038').innerText(),/97155/);assert.equal(await page.locator('.adaptive-home-v1038.no-load').count(),0);}else{await page.locator('.adaptive-home-v1038.no-load').waitFor();assert.equal(await page.locator('.adaptive-mission-v1038').count(),0);}assert.deepEqual(errors,[]);await page.screenshot({path:`${output}/${name}-${kind}.png`,fullPage:false});reports.push({browser:name,kind,passed:true});console.log(`PASS — ${name} ${kind}: ${expectedActive?'Rate Con guide owns Active Load':'log/route fallback stays inactive'}`);}catch(error){await page.screenshot({path:`${output}/${name}-${kind}-FAILED.png`,fullPage:false}).catch(()=>{});reports.push({browser:name,kind,passed:false,error:String(error),stack:error.stack,pageErrors:errors});console.error(error);}finally{await context.close();}
  }

...
import {processDocumentClient} from './v3/processDocumentClientV110330.js';
...
function Glyph({name}) {
  const paths={camera:<><path d="M8 5 6 8H3v12h18V8h-3l-2-3Z"/><circle cx="12" cy="14" r="4"/></>,photos:<><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8" cy="8" r="1"/><path d="m3 17 5-5 4 4 4-6 5 7"/></>,file:<><path d="M14 3H5v18h14V8Z M14 3v5h5 M8 12h8 M8 16h6"/></>,back:<path d="m15 5-7 7 7 7"/>,plus:<path d="M12 5v14 M5 12h14"/>,adjust:<path d="M6 3v15h15 M3 6h15v15"/>,rotate:<path d="M4 10a8 8 0 1 1 1 8 M4 4v6h6"/>,order:<path d="M8 3v18 M4 7l4-4 4 4 M16 21V3 M12 17l4 4 4-4"/>,delete:<path d="M3 6h18 M9 6V3h6v3 M6 6l1 15h10l1-15 M10 10v7 M14 10v7"/>,check:<path d="m5 12 4 4 10-10"/>};
...
function FullPagePreview({file,onClose}) {
  const [enlarged,setEnlarged]=useState(false),dialog=useRef(null),viewport=useRef(null);
  useEffect(()=>{
    const opener=document.activeElement,root=dialog.current;root.querySelector('button')?.focus();
    const key=event=>{if(event.key==='Escape'){event.preventDefault();onClose();}if(event.key==='Tab'){const controls=root.querySelectorAll('button,[tabindex="0"]'),first=controls[0],last=controls[controls.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}}};
    root.addEventListener('keydown',key);return()=>{root.removeEventListener('keydown',key);opener?.focus?.();};
  },[]);
  function toggle(){setEnlarged(value=>!value);viewport.current?.scrollTo(0,0);}
  return <div ref={dialog} className="scan-zoom-v328 scan-zoom-v333" role="dialog" aria-modal="true" aria-label="Full page preview"><header><button type="button" onClick={onClose}>Close preview</button><button type="button" aria-pressed={enlarged} onClick={toggle}>{enlarged?'Fit page':'Zoom in'}</button></header><div ref={viewport} className={enlarged?'scan-zoom-detail-v333':'scan-zoom-fit-v333'} tabIndex="0"><Thumbnail file={file} alt="Full document page"/></div><p>{enlarged?'Scroll to inspect the small print.':'The whole page · Zoom in for small print'}</p></div>;
}
...
export default function ScanIntakeV110328({onReady,onClose,initialDraft}) {
  const [pages,setPages]=useState(()=>initialDraft?.pages||[]),[selected,setSelected]=useState(()=>initialDraft?.pages?.[0]?.id||''),[mode,setMode]=useState('pages'),[crop,setCrop]=useState(null),[busy,setBusy]=useState(false),[status,setStatus]=useState(''),[error,setError]=useState(''),[pdfCount,setPdfCount]=useState(0),[zoom,setZoom]=useState(false),[reordering,setReordering]=useState(false);
  ...
  async function addFiles(selection,fromCamera=false) {
    ...
    try{
      ...
      for(const file of files){
        let preview=null,result=null,corners=FULL_PAGE;
        if(file.type.startsWith('image/')){
          setStatus(`Preparing photo ${added.length+1} of ${files.length}…`);
          try {
            const processed=await processDocumentClient(file,fromCamera?'camera':'photo-intake');
            if(token!==generation.current)return;
            result=processed.result;corners=processed.corners;
            preview=result.displayFile;
          }
          catch{throw new Error(`“${file.name}” could not be opened. Try a JPG, PNG or a new photo.`);}
        }
        if(token!==generation.current)return;
        added.push({id:crypto.randomUUID(),file,preview,result,corners,rotation:0});
      }
      setPages(previous=>[...previous,...added]);setSelected(added[0].id);setPdfCount(0);if(!fromCamera)setMode('pages');
    }catch(cause){if(token===generation.current)setError(cause.message||'The file could not be opened. Try another copy.');if(fromCamera)throw cause;}
  ...
  async function editPage(){
    ...
    try{
      ...
      if(token!==generation.current)return;
      // Reopening keeps the selected crop, including an explicit Full page reset.
  ...
  async function rotatePage(){
    if(!current||busyRef.current)return;
    const token=start('Rotating page…');
    try{
      let session=await scannerEngineV3.prepare(current.file,{source:'page-review'});
      for(let i=0;i<(current.rotation||0)/90;i++)session=await scannerEngineV3.rotate(session);
      session.corners=current.corners||FULL_PAGE;session=await scannerEngineV3.rotate(session);
      const result=await scannerEngineV3.finalize(session,session.corners,{preserveOrientation:true});
      if(token!==generation.current)return;
      setPages(previous=>previous.map(page=>page.id===current.id?{...page,result,corners:session.corners,rotation:session.rotation}:page));
  ...
  async function applyCrop(corners){
    ...
    try{const result=await scannerEngineV3.finalize(crop.session,corners,{preserveOrientation:true});if(token!==generation.current)return;
  ...
  async function readDocument(){
    ...
    try{
      ...
      for(let i=0;i<pages.length;i++){
        ...
        if(!result){const session=await scannerEngineV3.prepare(page.file,{source:'photo-intake'});if(token!==generation.current)return;result=await scannerEngineV3.finalize(session,page.corners||FULL_PAGE,{preserveOrientation:true});}
        if(token!==generation.current)return;
  ...
  if(mode==='camera')return <CameraAdapterV3 pageCount={pages.length} maxPages={MAX_PHOTO_PAGES} lastPage={pages.at(-1)?.result?.displayFile||pages.at(-1)?.preview} onCancel={()=>setMode('pages')} onCapture={file=>addFiles([file],true)}/>;
  ...
  return <section className={`scan-intake-v328${pages.length&&isImage?' scan-review-v333':''}`} aria-label="Document scanner" aria-busy={busy}>
    <header className="scan-intake-head-v328"><button type="button" className="scan-icon-v328" onClick={close} aria-label="Close scanner"><Glyph name="back"/></button><div><span>ROAD READY</span><b>{pages.length?'Review pages':'Smart Scan'}</b></div><span className="scan-private-v328"><Glyph name="check"/> On device</span></header>
    <main>
      {!pages.length&&<nav className="scan-stages-v328" aria-label="Scan progress"><b aria-current="step"><span>1</span>Pages</b><i/><span><em>2</em>Read & review</span><i/><span><em>3</em>Save</span></nav>}
      {!pages.length?<>
        ...
        {isImage?<>
          ...
          <div className="scan-page-tools-v328"><button type="button" disabled={busy} onClick={editPage} aria-label="Crop & rotate"><Glyph name="adjust"/>Adjust</button><button type="button" disabled={busy} onClick={rotatePage} aria-label="Rotate selected page"><Glyph name="rotate"/>Rotate</button><button type="button" disabled={busy||pages.length<2} aria-label="Reorder pages" aria-pressed={reordering} onClick={()=>setReordering(!reordering)}><Glyph name="order"/>Order</button><button type="button" disabled={busy} onClick={()=>remove(current.id)} aria-label="Remove page"><Glyph name="delete"/>Delete</button></div>
          <ol className={`scan-page-list-v328${reordering?' scan-reordering-v333':''}`} aria-label="Document page order">{pages.map((page,index)=><li key={page.id} aria-current={page.id===current.id?'true':undefined}><button type="button" disabled={busy} onClick={()=>setSelected(page.id)} aria-label={`Select page ${index+1}`}><Thumbnail file={page.result?.displayFile||page.preview}/><span><b>Page {index+1}</b><small>{page.file.name}</small></span></button>{reordering&&<div><button type="button" aria-label={`Move page ${index+1} earlier`} disabled={busy||index===0} onClick={()=>setPages(previous=>movePage(previous,page.id,-1))}>↑</button><button type="button" aria-label={`Move page ${index+1} later`} disabled={busy||index===pages.length-1} onClick={()=>setPages(previous=>movePage(previous,page.id,1))}>↓</button></div>}</li>)}</ol>
    ...
    {zoom&&<FullPagePreview file={preview} onClose={()=>setZoom(false)}/>}
  </section>;
}

...
for (const token of ['--rr-bg','--rr-surface','--rr-text','--rr-blue','--rr-green','--rr-safe-bottom']) {
  assert.ok(css.includes(token), `Missing design token ${token}`);
}
for (const selector of ['.command-home-head','.command-ready-card','.command-next-card','.command-load-card','.command-module-grid','.logbook-screen','.business-screen','.bottom-nav']) {
...
const client = fs.readFileSync('source/src/app/road-ready-client.jsx', 'utf8');
for (const needle of ['Enter total miles for this driving','Speed guide for this leg','Location estimate','Total driving miles missing']) {
  const index = client.indexOf(needle);
  console.log(`\n=== MILEAGE PROMPT ${needle} @ ${index} ===`);
  if (index >= 0) console.log(client.slice(Math.max(0, index - 2200), Math.min(client.length, index + 4200)));
}

...
import {validBoundary} from './v3/documentBoundaryV110329.js';
    ...
    try{
      ...
      for(const file of files){
        ...
        if(file.type.startsWith('image/')){
          ...
          try {
            const session=await scannerEngineV3.prepare(file,{source:fromCamera?'camera':'photo-intake'});
            if(token!==generation.current)return;
            // Detect on the actual still photo: video and still-photo dimensions
            // can differ on iOS. Never copy a stale live-video quadrilateral.
            if(session.detection?.found&&session.detection.confidence>=.80&&validBoundary(session.detection.corners))corners=session.detection.corners;
            result=await scannerEngineV3.finalize(session,corners,{preserveOrientation:true});

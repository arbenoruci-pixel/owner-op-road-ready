...
export default function CameraAdapterV3({onCapture,onCancel,pageCount=0,maxPages=20,lastPage=null}){
  const videoRef=useRef(null),containerRef=useRef(null),streamRef=useRef(null),nativeInput=useRef(null),alive=useRef(true),busy=useRef(false),autoRef=useRef(true),captureRef=useRef(null),tracking=useRef(null),qualitySince=useRef(null),latched=useRef(false),misses=useRef(0),limit=useRef(false);
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      if(video?.videoWidth&&video.readyState>=2&&!busy.current&&!document.hidden){try{
        ...
        const eligible=detection.found&&quality.ready&&tracking.current.movement<.025;
        if(!eligible)qualitySince.current=null;else if(qualitySince.current===null)qualitySince.current=now;
        setReady(eligible);
        ...
        if(!limit.current&&!latched.current&&autoRef.current&&qualitySince.current!==null&&now-qualitySince.current>=900)captureRef.current?.();
      }catch{tracking.current=null;qualitySince.current=null;setReady(false);setCorners(null);}}
  ...
  async function deliver(file){
    ...
    if(!alive.current)return;latched.current=true;misses.current=0;tracking.current=null;qualitySince.current=null;setCorners(null);setReady(false);setGuidance('Page added. Show the next page, or tap Capture.');
  }
  async function capture(){
    ...
    try{
      ...
      if(!file){let best=null,bestScore=-Infinity;try{
        for(let i=0;i<3;i++){await nextFrame(video);if(!alive.current)return;const candidate=frameCanvas(video),small=frameCanvas(video,800),ctx=small.getContext('2d',{willReadFrequently:true}),pixels=ctx.getImageData(0,0,small.width,small.height);const q=assessDocumentQuality(pixels,{corners:tracking.current?.corners,nativeWidth:candidate.width,nativeHeight:candidate.height});small.width=small.height=1;if(q.score>bestScore){if(best)best.width=best.height=1;best=candidate;bestScore=q.score;}else candidate.width=candidate.height=1;}
  ...
  return <section data-smart-camera="110329" style={{position:'fixed',inset:0,zIndex:1400,background:'#020617',color:'#fff',display:'grid',gridTemplateRows:'auto 1fr auto'}}>
    ...
    <footer style={{padding:'10px 12px calc(12px + env(safe-area-inset-bottom))',display:'grid',gap:10}}>
      <div style={{display:'flex',gap:8,justifyContent:'center'}}>
        <button type="button" disabled={capturing} aria-pressed={auto} onClick={()=>{autoRef.current=!auto;setAuto(!auto);qualitySince.current=null;}} style={button}>Auto {auto?'on':'off'}</button>

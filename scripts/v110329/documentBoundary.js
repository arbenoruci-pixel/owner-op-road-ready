...
function sampleImage(image,maxSide) {
  ...
  const rgb=new Float32Array(w*h*3),gray=new Float32Array(w*h),neutral=new Float32Array(w*h);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    ...
    const r=rgb[i*3],g=rgb[i*3+1],b=rgb[i*3+2];gray[i]=r*.2126+g*.7152+b*.0722;neutral[i]=gray[i]-(Math.max(r,g,b)-Math.min(r,g,b))*.7;
  }
  ...
  return {w,h,rgb,gray,blur:smooth(gray),neutral:smooth(neutral)};
}
...
function scoreQuad(points,frame,fill) {
  ...
  for(let side=0;side<4;side++){
    ...
    for(let k=1;k<=24;k++){
      ...
      contrast=Math.max(contrast,color*.8);if(contrast>11)strong++;total+=Math.min(70,contrast);
    }
  ...
  if(ink<.004||ink>.36||coverage<.12)return null;
  const rectangularity=clamp(fill/(a*(w-1)*(h-1))),paperMean=mean/Math.max(1,samples);
  const score=.35*support+.20*weakest+.15*clamp(coverage/.55)+.13*Math.sqrt(a)+.10*clamp(ink/.04)+.07*clamp((paperMean-60)/130);
  return {corners:quad,confidence:clamp(score),metrics:{paperArea:a,edgeSupport:support,weakestEdge:weakest,inkRatio:ink,inkCoverage:coverage,paperMean,rectangularity}};
}
function lineQuads(frame,visit) {
  const {w,h,blur}=frame,diag=Math.ceil(Math.hypot(w,h)),bins=diag*2+1,angles=60,votes=new Float32Array(bins*angles),edges=[];
  ...
  for(let y=2;y<h-2;y++)for(let x=2;x<w-2;x++){
    const i=y*w+x,gx=blur[i+1]-blur[i-1],gy=blur[i+w]-blur[i-w],mag=Math.hypot(gx,gy);if(mag<16)continue;
...
export function detectDocumentBoundary(image,options={}) {
  ...
  const thresholds=[85,115,145,175,200,220];
  for(const values of [frame.blur,frame.neutral])for(const threshold of thresholds){
  ...
  // Keep a small paper margin rather than trimming faint notes at the boundary.
  const cx=best.corners.reduce((n,p)=>n+p.x,0)/4,cy=best.corners.reduce((n,p)=>n+p.y,0)/4;
  const corners=best.corners.map(p=>({x:clamp(cx+(p.x-cx)*1.018),y:clamp(cy+(p.y-cy)*1.018)}));
  return {...best,corners,found:true,method:'region-quad-edge-support-v110329',metrics:{...best.metrics,candidateCount:candidates.length,analysisWidth:frame.w,analysisHeight:frame.h},...(options.debug?{candidates:candidates.slice(0,15)}:{})};
}

...
const lineWidth = Number((graph.match(/const\s+LINE_W\s*=\s*([0-9.]+)/) || [])[1]);
ok(Number.isFinite(lineWidth) && lineWidth >= 5.25 && lineWidth <= 5.85, 'graph line width is readable and still not blocky');
...
ok(graph.includes('stroke={major ? \'#cfd8e3\' : \'#edf2f7\'}') || graph.includes('HOUR_GRID_COLOR') && graph.includes('QUARTER_GRID_COLOR'), 'grid is lightened');
ok(graph.includes('strokeWidth={major ? 0.72 : 0.34}') || graph.includes('strokeWidth={major ? HOUR_GRID_W : QUARTER_GRID_W}'), 'grid lines are fine and narrow');
ok(graph.includes('r={selected ? 7 : 5.5}') || graph.includes('short-event-trace-boost'), 'short events remain readable without noisy markers');

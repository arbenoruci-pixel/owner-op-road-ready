...
import {installAngledReaderV110339} from './v110339/install.mjs';
installAngledReaderV110339();
const VERSION='110.3.39',BUILD='v110339-angled-page-reader';
for(const path of ['release-version.json','public/app-version.json']){
  const value=JSON.parse(fs.readFileSync(path,'utf8'));
  Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.39 Straighten captured pages',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,notes:['Keep all four detected corners when straightening tilted pages.','Separate adjacent field values in OCR rows.','Keep uncertain readings attached to their exact page evidence.']});
...
const before="assert.equal(meta.version,'110.3.38');assert.equal(meta.build,'v110338-reader-document-packets');";
...
console.log('PASS — v110.3.39 tilted page rectification and reader field boundaries installed');

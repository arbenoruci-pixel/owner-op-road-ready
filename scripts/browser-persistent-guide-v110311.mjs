...
const output=`browser-test-results/persistent-guide-${process.env.TEST_ORIGIN?'production':'local'}`;
...
for(const[name,type]of[['chromium',chromium],['webkit',webkit]]){
 ...
 try{
  for(const [kind,state,expectedActive] of [['instruction-recovery',instructionState(),true]]){
...
assert.deepEqual(errors,[]);await page.screenshot({path:`${output}/${name}-${kind}.png`,fullPage:false});reports.push({browser:name,kind,passed:true});console.log(`PASS — ${name} ${kind}: ${expectedActive?'Instruction guide survives Home, checklist and reload':'log/route fallback stays inactive'}`);}catch(error){await page.screenshot({path:`${output}/${name}-${kind}-FAILED.png`,fullPage:false}).catch(()=>{});reports.push({browser:name,kind,passed:false,error:String(error),stack:error.stack,pageErrors:errors});console.error(error);}finally{await context.close();}
  }

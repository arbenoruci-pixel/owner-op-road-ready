...
// Narrow contextual fallback for a weak scanner result during an active pickup.
{
...
// Forced update state cannot be dismissed after this build is installed.
{
...
for(const path of ['release-version.json','public/app-version.json']){
  const meta=JSON.parse(read(path));Object.assign(meta,{version:VERSION,build:BUILD,force:true,releasedAt,updatedAt:releasedAt,label:'v110.3.5 Live BOL Context Recovery',notes:['Weak scanner results during the active pickup are re-evaluated against the current live BOL before confirmation.','Current live BOL identity can supply load/date only for weak Rate Con/Other results; strong document evidence remains isolated.','Forced PWA updates cannot be dismissed after this build is installed.']});write(path,JSON.stringify(meta,null,2)+'\n');
}

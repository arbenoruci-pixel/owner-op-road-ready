...
test('Owner Operator client references only isolated prototype Supabase',()=>{const s=fs.readFileSync('lib/owner-op-cloud/client.js','utf8');assert.ok(s.includes('ghwkcgczuwctzxsxmqzx.supabase.co'));assert.ok(!s.includes('vnidjrxidvusulinozbn'));assert.ok(s.includes('sb_publishable_'));assert.ok(s.includes('detectSessionInUrl: true'));});
...
test('password recovery uses verified session and preserves approval gate',()=>{const s=fs.readFileSync('source/src/modules/auth/AuthGate.jsx','utf8');assert.ok(s.includes('resetPasswordForEmail'));assert.ok(s.includes("event === 'PASSWORD_RECOVERY'"));assert.ok(s.includes('updateUser({ password })'));assert.ok(s.includes("setMode('signin')"));assert.ok(!s.includes('service_role'));});

...
const VERSION='110.3.40',BUILD='v110340-page-reading-evidence';
for(const path of ['release-version.json','public/app-version.json']){
  const value=JSON.parse(fs.readFileSync(path,'utf8'));
  Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.40 Recognize noisy document readings',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,notes:['Recognize document headings despite scan punctuation.','Combine complementary type evidence from reads of the same page.','Keep uncertain amounts, dates and document types available for review.']});
...
const before="assert.equal(meta.version,'110.3.39');assert.equal(meta.build,'v110339-angled-page-reader');";
...
console.log('PASS — v110.3.40 noisy document classification and same-page evidence installed');

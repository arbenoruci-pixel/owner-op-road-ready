...
import {detectDocumentBoundary,FULL_FRAME} from './v110334/documentBoundary.js';
...
function fixture({seed=1,tint=[211,211,204],plain=false,blank=false,background='mixed',shadow=.20,seam=false}={}){
  ...
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    ...
    if(seam&&y>=532&&y<543)rgb=[44,40,35];
    if(inside){
...
for(const options of [{},{seam:true},{seam:true,seed:3},{seed:3},{seed:11,shadow:.12},{seed:17,shadow:.26},{background:'light'},{background:'dark'},{background:'dark',tint:[224,160,146]}]){
...
// A board/upholstery edge continues below the sheet. The previous detector
// chose that outer rectangle, adding 30–50 pixels of background to the crop.
for(const bottom of [342,360])for(const inset of [0,2,5]){
 const width=300,height=400,data=new Uint8ClampedArray(width*height*4),expected=[{x:60+inset,y:55+inset},{x:240-inset,y:55+inset},{x:240-inset,y:310},{x:60+inset,y:310}];
 for(let y=0;y<height;y++)for(let x=0;x<width;x++){
  const n=((x*71+y*137+x*y*7)%53)/53;let rgb=[60+n*45,52+n*40,45+n*30];
  if(x>=60&&x<=240&&y>=55&&y<=bottom)rgb=[174,156,130];
  if(x>=60+inset&&x<=240-inset&&y>=55+inset&&y<=310){rgb=[216,213,205];if(x>75&&x<225&&y>77&&y<273&&y%19<3&&x%29<24)rgb=[55,52,51];if(y>295&&y<302&&x>65&&x<235)rgb=[32,32,32];}
  data.set([...rgb,255],(y*width+x)*4);
 }
 const found=detectDocumentBoundary({width,height,data});assert.ok(found.found);
 const error=Math.max(...found.corners.map((p,i)=>Math.hypot(p.x*(width-1)-expected[i].x,p.y*(height-1)-expected[i].y)));
 assert.ok(error<7,'paper edge must win over supporting surface: '+JSON.stringify({bottom,inset,error}));
}
console.log('PASS — six outer-surface regressions retain the paper footer and exclude the surface beneath it');

...
const VERSION='110.0.1',BUILD='v110001-approved-email-login';
...
fs.writeFileSync('release-version.json',JSON.stringify({version:VERSION,build:BUILD,label:'Approved email login and isolated private cloud'},null,2)+'\n');
const previous=JSON.parse(fs.readFileSync('public/app-version.json','utf8'));
fs.writeFileSync('public/app-version.json',JSON.stringify({...previous,version:VERSION,build:BUILD,label:'Approved email login and isolated private cloud',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),force:false,notes:['Road Ready stays locked until a confirmed email session is manually approved.','Owner Operator now authenticates only against the isolated owner-op-prototype Supabase project.','Approved devices retain a seven-day offline grace after a successful online access verification.']},null,2)+'\n');
...
console.log('PASS — isolated Owner Operator cloud and approved email gate assembled; Tepiha project is not referenced');

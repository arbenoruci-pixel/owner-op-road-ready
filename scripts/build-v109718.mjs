...
const filtered=scripts.filter(name=>!['apply-v109718-actual-load-day-evidence.mjs','verify-v109718-actual-load-day-evidence.mjs'].includes(name));
filtered.push('apply-v109718b-actual-load-day-evidence.mjs','verify-v109718-actual-load-day-evidence.mjs');
for(const script of filtered)run(process.execPath,[`scripts/${script}`]);
fs.rmSync('.next',{recursive:true,force:true});
run('npx',['next','build']);
run(process.execPath,['scripts/verify-v109717-built-production-bundle.mjs']);

...
function replaceRegex(content, pattern, after, marker, label) {
  if (marker && content.includes(marker)) return content;
  if (!pattern.test(content)) throw new Error(`v106.2 missing ${label}`);
...
if (!web.includes('paper-text-fusion-v1062')) {
  ...
  web = replaceRegex(
    web,
    /if\s*\(\s*candidate\s*\)\s*candidates\.push\(\s*candidate\s*\);/,
    "if (candidate && isPlausibleDocumentCandidateV106(candidate, 'rectangle-detection')) candidates.push(candidate);",
    "isPlausibleDocumentCandidateV106(candidate, 'rectangle-detection')",
  ...
  web = replaceRegex(
    web,
    /if\s*\(\s*saliency\s*\)\s*candidates\.push\(\s*saliency\s*\);/,
    "if (saliency && isPlausibleDocumentCandidateV106(saliency, 'saliency')) candidates.push(saliency);",
    "isPlausibleDocumentCandidateV106(saliency, 'saliency')",

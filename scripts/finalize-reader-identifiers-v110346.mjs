...
const VERSION='110.3.46',BUILD='v110346-identifier-fragments';
for(const path of ['release-version.json','public/app-version.json']){
  const value=JSON.parse(fs.readFileSync(path,'utf8'));
  Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.46 Recheck incomplete document numbers',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,notes:['Keep partial and competing BOL readings uncertain.','Show source-backed numeric alternatives for confirmation.','Retry weak references and long Bill of Lading number labels.']});
...
const before="assert.equal(meta.version,'110.3.45');assert.equal(meta.build,'v110345-paper-quality-recovery');";
...
console.log('PASS — v110.3.46 incomplete-reference checks and long-label rereads installed');

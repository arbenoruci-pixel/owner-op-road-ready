...
fs.copyFileSync('packages/smart-reader-core/src/fieldGuards.js','source/src/modules/scan/documentFieldGuardsV110336.js');
const VERSION='110.3.41',BUILD='v110341-merged-reading-fields';
for(const path of ['release-version.json','public/app-version.json']){
  const value=JSON.parse(fs.readFileSync(path,'utf8'));
  Object.assign(value,{version:VERSION,build:BUILD,force:false,label:'v110.3.41 Read merged receipt and shipping fields',releasedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,notes:['Read receipt details when OCR merges neighboring columns.','Recognize customer PO labels and exclude form labels from company names.','Preserve source evidence and unresolved OCR disagreements.']});
...
const before="assert.equal(meta.version,'110.3.40');assert.equal(meta.build,'v110340-page-reading-evidence');";
...
console.log('PASS — v110.3.41 merged receipt fields and shipping label guards installed');

...
assert.match(dot,/cloudApi\(\{ action:'catalog' \}\)/);
assert.match(dot,/missingDatesV11028 = days\.filter/);
assert.match(dot,/Roadside cloud package is missing:/);
assert.match(dot,/Number\(share\?\.log_count \|\| 0\) !== days\.length/);
assert.match(dot,/cloudApi\(\{ action:'create_share', wallet_keys:\[\] \}\)/);
assert.doesNotMatch(dot,/const walletKeys = Object\.keys\(state\.dotWallet/);
assert.match(dot,/8-day logs only/);
...
console.log('PASS — officer enters email, Road Ready requires all eight cloud log dates, shares logs only through a private capability link, and opens an addressed email without routing-code UI');

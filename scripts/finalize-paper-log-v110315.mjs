...
`+s.slice(b));
 ...
 patch(edit,'{liveV110 ? <div className="editor-live-heading-v110"><strong>{DUTY_SHORT_LABELS[status]} <span>LIVE</span></strong><button type="button" onClick={changeLiveStatusV110}>Change status</button></div> : <EditorDutyStatusControls status={status} onChange={changeStatus} />}','<div className="paper-duty-options-v110315"><EditorDutyStatusControls status={status} onChange={changeStatus} /></div>');
...
patch('source/src/core/timeline/manualDrivingContinuity.js',"  if (!lastPrevious || lastPrevious.status !== 'D') return state;","  if (!lastPrevious || lastPrevious.status !== 'D' || lastPrevious.paperLogEndV110315) return state;");

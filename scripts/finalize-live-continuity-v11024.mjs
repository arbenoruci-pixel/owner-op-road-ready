...
assert.ok(graph.includes('compact-graph-toolbar-minimal-v11024'),'compact graph toolbar contract missing');
...
for(const p of ['release-version.json','public/app-version.json']){
  ...
  Object.assign(meta,{version:VERSION,build:BUILD,force:false,sourceCommit:process.env.VERCEL_GIT_COMMIT_SHA||process.env.GITHUB_SHA||null,
    label:'Connected live duty timeline + compact quick activities',
    notes:['Current OFF/SB/ON/Driving projects continuously to Now until the next status change.','Manual override and automatic Driving protections remain unchanged.','Helper copy is compacted so quick activities stay visible higher on the phone.']});

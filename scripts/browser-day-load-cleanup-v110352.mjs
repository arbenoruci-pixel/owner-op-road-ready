...
for(const [name,type] of [['chromium',chromium],['webkit',webkit]]){
 ...
 try{
  ...
  // Deleting the last actual row is the existing way to clear this Logbook day.
  ...
  await editor.locator('.sheet-head button').last().click();
  const removed=await waitState(page,s=>!(s.eventsByDay?.[day]||[]).some(e=>e.id==='fixture-event'));
  ...
  assert.ok(!(reloaded.eventsByDay[day]||[]).some(e=>e.id==='fixture-event'));
  assert.deepEqual(errors,[]);
  await page.screenshot({path:'browser-test-results/day-load-cleanup-'+name+'.png',fullPage:true});
  console.log('PASS — '+name+': last-event delete clears owned load/routes offline and after reload; other-day evidence preserved');
 }catch(error){await page.screenshot({path:'browser-test-results/day-load-cleanup-'+name+'-failed.png',fullPage:true}).catch(()=>{});fs.writeFileSync('browser-test-results/day-load-cleanup-'+name+'-failure.json',JSON.stringify({error:String(error),errors,dialogs,text:await page.locator('body').innerText()},null,2));throw error;}

...
function render(s,start,end,label) {
 ...
 const expected=s.eventsByDay[s.activeDay].map(e=>e.status==='D'&&e.startMin===start?{...e,endMin:end}:e);
 const path=traceGeometry(expected).segments.find(s=>s.event.status==='D'&&s.event.startMin===start)?.path;
 assert.ok(path,'expected Driving segment exists');
 assert.ok(svg.includes('d="'+path+'"'),label+' Driving path: '+path+' actual: '+JSON.stringify([...svg.matchAll(/class="duty-trace-v110"[^>]* d="([^"]+)"/g)].map(m=>m[1])));

// Synthetic isolated compiled-app tests; all external account writes blocked.
...
function fixture(kind) {
 const drive=row('drive','D',1325,1326,{...(kind==='manual-end'?{paperLogEndV110315:true}:{})});
 const bridge=row('bridge','D',0,42,{source:'manual_drive_midnight_continuation',crossMidnightContinuation:true,crossMidnightFromDay:day,crossMidnightFromEventId:kind==='unlinked'?'different-trip':'drive'});
 return {view:'day',activeDay:day,sheet:null,selectedEventId:null,selectedIds:[],selectMode:false,homeTerminalTimeZone:'America/New_York',
 driver:{truck:'TEST',trailer:'TEST'},driverProfile:{name:'Synthetic Driver'},carrierName:'Synthetic Carrier',mainOfficeAddress:'Test Office',currentTrailer:'TEST',
 currentStatus:'SB',currentReason:'Sleeper',currentLocation:{city:'Willowbrook',state:'IL'},
 eventsByDay:{[day]:[row('sleep','SB',0,1315),row('pti','ON',1315,1325),drive],[next]:[bridge,row('now','SB',42,43,{source:'live_status'})]},
 manualDrivingSession:{active:false,status:'D',eventId:'bridge',startDay:next,endedDay:next,endedMin:42,endedAt:'2026-09-18T04:42:00Z'},
 certifyStatus:{[day]:'Needs signature'},signatureByDay:{},logbookEditHistoryByDay:{},inspectionByDay:{},routeLegsByDay:{},formByDay:{},loadGuidesById:{},dotWallet:{documents:{}}};
}
...
async function setup(page,context,state) {
 await context.route('**/*',async route=>{
  ...
  if(url.origin===origin){if(url.pathname.startsWith('/api/'))return route.fulfill({json:{},status:200});assert.equal(route.request().method(),'GET');return route.continue();}
  ...
  return route.fulfill({status:403,json:{error:'Synthetic test: external access blocked'},headers});
 });
 await page.clock.setFixedTime(new Date('2026-09-18T04:50:00Z'));await page.goto(origin+'/_not-found');
 await page.evaluate(async({schemas,state,session})=>{
  ...
  await new Promise((resolve,reject)=>{const r=indexedDB.open('owner-op-road-ready-offline-v1',20);
   r.onupgradeneeded=()=>{const db=r.result;for(const [name,schema]of Object.entries(schemas)){const [key,...indexes]=schema.split(',').map(s=>s.trim()),st=db.createObjectStore(name,{keyPath:key.replace(/^&/,'')});for(const raw of indexes){const field=raw.replace(/^[&*]/,'');st.createIndex(field,field.startsWith('[')?field.slice(1,-1).split('+'):field,{unique:raw.startsWith('&'),multiEntry:raw.startsWith('*')});}}};
   r.onerror=()=>reject(r.error);r.onsuccess=()=>{const db=r.result,tx=db.transaction('app_snapshots','readwrite');tx.objectStore('app_snapshots').put({key:'owner-op-road-ready-state-v1',state,updated_at:new Date().toISOString()});tx.oncomplete=()=>{db.close();resolve();};tx.onerror=()=>reject(tx.error);};
  });
...
for(const [name,type]of [['chromium',chromium],['webkit',webkit]]){
 ...
 for(const kind of ['linked','manual-end','unlinked']){
  ...
  try{
   await setup(page,context,fixture(kind));const before=await stored(page);
   assert.equal(before.activeDay,day,'the selected log stays on its original date');
   const end=kind==='linked'?1440:1326;await assertEnd(page,end);
   await page.screenshot({path:`${output}/${name}-${kind}.png`});
   await page.reload();await openLog(page);await assertEnd(page,end);
   const after=await stored(page);
   for(const key of ['eventsByDay','signatureByDay','logbookEditHistoryByDay'])assert.deepEqual(after[key],before[key],key+' unchanged by reopen');
   assert.deepEqual(errors,[]);reports.push({browser:name,kind,end,passed:true,storedOriginEnd:before.eventsByDay[day].find(e=>e.id==='drive')?.endMin});
   console.log(`PASS — ${name} ${kind}: exact originating-day endpoint ${end}, reopen and immutable saved history`);
  }catch(error){await page.screenshot({path:`${output}/${name}-${kind}-FAILED.png`}).catch(()=>{});fs.writeFileSync(`${output}/${name}-${kind}-state.json`,JSON.stringify(await stored(page).catch(()=>null),null,2));reports.push({browser:name,kind,passed:false,error:String(error),stack:error.stack,errors});console.error(error);}

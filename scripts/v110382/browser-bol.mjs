...
const output='browser-test-results/bol-quality-v110382';fs.mkdirSync(output,{recursive:true});
const rows=[
  ...
  try{
    ...
    await context.addInitScript(rows=>{
      window.Tesseract={createWorker:async()=>({setParameters:async()=>{},terminate:async()=>{},recognize:async(file)=>{
        ...
        for(const [i,row]of rows.entries())for(const [j,word]of (row.words||[[row.text,row.x,row.w]]).entries())tsv.push([5,1,i+1,1,1,j+1,x(word[1]),y(row.y),x(word[2]),y(24),96,word[0]].join('\t'));
        return {data:{text:rows.map(r=>r.text).join('\n'),confidence:96,tsv:tsv.join('\n')}};
      }})};
    },rows);
    ...
    const png=await page.evaluate(rows=>{
      ...
      for(const row of rows)c.fillText(row.text.replace('BOL NO: 001234500','BOL NO: 0012345000'),row.x,row.y+24);
      let left=620;
    ...
    console.log('PASS '+name+' actual barcode, crop proof, BOL correction, unit guard and exported evidence');

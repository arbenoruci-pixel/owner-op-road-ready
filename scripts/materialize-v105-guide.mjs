...
if (!guide.includes('function eventMatchesGuideReferenceV105')) {
  ...
  const replacement = [
    ...
    "}",
    "",
    "function completedInspectionForStepV105(state = {}, guide = {}, step = {}) {",
    "  const day = dayKey(step.day || guide.pickupDate || '') || step.day || guide.pickupDate || '';",
    "  if (!day) return false;",
    "  const inspection = state.inspectionByDay?.[day] || null;",
    "  if (!inspection || !(inspection.complete || inspection.status === 'complete')) return false;",
    "  const sourceEventId = String(inspection.sourceEventId || '').trim();",
    "  if (!sourceEventId) return true;",
    "  const sourceEvent = (state.eventsByDay?.[day] || []).find(event => event?.id === sourceEventId);",
    "  if (!sourceEvent) return true;",
    "  return sourceEvent.status === 'ON' && /pre[- ]?trip|inspection/i.test(`${sourceEvent.note || ''} ${sourceEvent.description || ''}`);",
    "}",
    "",
    ...
    "  if (pretripReason && completedInspectionForStepV105(state, guide, step)) return true;",

...
`,
  `  if (!hasOnlyContiguousIndexes(indexes)) {
    // v97.0: allow any selected events to move. Split a disjoint selection into
    // contiguous groups, then move each group in a direction-safe order. Moving
    // later starts with the last group; moving earlier starts with the first.
    // If any group cannot move, roll back the whole operation so the user never
    // gets a partial one-hour shift.
    const groups = [];
    let current = [];
...
write('public/app-version.json', `${JSON.stringify({
  ...
  notes:[
    'Lets any selected duty events move together, including selections separated by unselected events.',
    'Processes selected groups in a direction-safe order so a one-hour move can free space instead of silently doing nothing.',
    'Rolls back the full move if any selected group cannot move, preventing partial or inconsistent edits.',
    'Keeps event durations, unselected event identity, HOS recalculation, recertification, and undo behavior intact.'
  ],
...
if (!timeline.includes("mode:'disjoint_selected_groups'") || !timeline.includes('roll back the whole operation')) {
  throw new Error('v97.0 verification failed');
}

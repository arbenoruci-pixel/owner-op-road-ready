...
if (!source.includes('const automaticFirstV1067 = selectPhotoFirstCandidateV1065(found)')) {
  const pattern = /const first = selectPhotoFirstCandidateV1065\(found\) \|\| found\[0\] \|\| null;\s*\n\s*setCandidates\(found\);\s*\n\s*setSelectedCandidateId\(first\?\.id \|\| ''\);\s*\n\s*setAutomaticContour\(first\?\.contour \|\| \[\]\);\s*\n\s*setContour\(first\?\.contour \|\| \[\]\);/;

...
ok(CURRENT_APP_VERSION === '95.58.0', 'current update version is v95.58.0');
ok(compareVersions('95.58.1', '95.58.0') > 0, 'version compare detects patch increase');
ok(compareVersions('95.59.0', '95.58.9') > 0, 'version compare detects minor increase');
ok(compareVersions('95.58.0', '95.58.0') === 0, 'version compare equal');
ok(isNewerVersion('95.59.0', '95.58.0'), 'isNewerVersion true for remote newer');
ok(!isNewerVersion('95.57.0', '95.58.0'), 'isNewerVersion false for remote older');
ok(normalizeRemoteVersionPayload({ version:'95.59.0', notes:['a'] }).version === '95.59.0', 'remote payload normalizes');

const versionJson = JSON.parse(read('public/app-version.json'));
ok(versionJson.version === '95.58.0', 'public app-version matches patch');
...
ok(sw.includes("OWNER_OP_SW_VERSION = '95.58.0'"), 'service worker version updated');
...
ok(pkg.version === '95.58.0', 'package version updated');
